...
class continents extends CI_Controller {
    ...
    public function index() {
        ...
        $config['base_url'] = base_url() . 'index.php/continents/index';
        ...
        $result = $this->continents_m->getAll($config['per_page'], $offset);
        $data['data']['results'] = $result;

        $data['data']['pagination_links'] = $this->pagination->create_links();
        $this->load->view('layout/admin/header');
        $data['viewpath'] = 'continents_view/index';
    ...
    public function add() {
        ...
        if ($this->form_validation->run()) {
            if ($this->continents_m->insert_continents($this->input->post()) != 0) {
                redirect('/continents', 'refresh');
            }
        }

        $this->load->view('layout/admin/header');
        $data['viewpath'] = 'continents_view/add_view';
    ...
    public function edit($id = 0) {
        ...
        $this->form_validation->set_rules($this->_validation_rules);
        if ($this->form_validation->run()) {
            if ($this->continents_m->update_continents($id, $this->input->post()) != 0) {
                redirect('/continents', 'refresh');
            }
        }
        $this->load->view('layout/admin/header');
        $data['viewpath'] = 'continents_view/add_view';
        $data['data'] = '';
        $data['results'] = $this->continents_m->getOne($id);
        $this->load->view('layout/admin/content', $data);
        $this->load->view('layout/admin/footer');
    }
